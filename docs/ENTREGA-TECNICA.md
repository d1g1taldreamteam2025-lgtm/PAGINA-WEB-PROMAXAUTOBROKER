...
⚠️ **En el remoto no existe la rama `main`**, solo dos ramas `claude/*`. Como
`deploy.yml` (GitHub Pages) y `deploy-hostinger.yml` (FTP) escuchan `push: branches: [main]`,
**nunca se ejecutan**. Y `inventory-sync.yml` está atado a la rama `claude/pensive-newton-3pumvq`.
Antes de entregar conviene consolidar todo en `main` y arreglar esos disparadores.
...
| `BOOKMARKLET_PASSWORD` | `admin/index.html:567`, `admin/install.html:130` | No (visible en DevTools) | Cambiar |
...
Secuencia obligatoria por dependencias. **Total estimado: 28–40 horas (4–5 días).**
...
| 4 | Repuntar el código (17 referencias) + regenerar bookmarklets | 4 h |
...
Después: `node tools/auto-sync/build-bookmarklets.js` (llevan la URL y la key embebidas).
...
1. **El respaldo del inventario está vacío.** `assets/data/inventory.json` contiene `[]`.
   `README` y `PANEL-ADMIN.md` lo venden como red de seguridad, pero no existe: si la base
   cae, `/inventory/` y los destacados del home quedan en blanco. **Volcar un snapshot real
   antes de cortar.**
...
3. **Había carpetas internas publicadas**: existe `.vercelignore` (excluye `docs/`,
   ...
   `CLAUDE.md` y `tools/` sí respondían 200 en producción. **Corregido en esta entrega**:
...
Están documentadas en `CLAUDE.md` y responden a pedidos explícitos del cliente:
